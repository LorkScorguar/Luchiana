...
Module utilisé pour
- envoyer des mails
- vérifier la présence de nouveaux mails non lus
"""
...
def sendTextMail(subject, content, dest):
    """Fonction d'envoie de mail en text brut"""
...
def checkGmail():
    """Fonction de vérification de nouveaux mails dans Gmail"""
...
def readMail(mid):
    ...
    mail.select("inbox")
    _, dec = mail.fetch(str(mid), "(RFC822)")
    message = dec[0][1].decode('utf-8')
...
def envoieMail(codeErreur=0, phrase="", infos=[]):
    ...
    if len(infos) > 1:
        ...
        mailD, mailS, mailC = "", "", ""
    if re.search(r"\w+@\w+\.\w{2,3}", phrase):
        regex = re.search(r"\w+@\w+\.\w{2,3}", phrase)
        mailD = regex.group(0).strip()
    if codeErreur == 1:
        mailD = phrase
    elif codeErreur == 2:
        mailS = phrase
